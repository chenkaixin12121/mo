 ...
 * @since 2023/12/04
 */
class CaptchaGrantAuthenticationProvider(
...
) : BaseAuthenticationProvider<CaptchaGrantAuthenticationToken>(authorizationService, tokenGenerator) {
    ...
    override fun supports(authentication: Class<*>): Boolean {
        return CaptchaGrantAuthenticationToken::class.java.isAssignableFrom(authentication)
    }

    override fun checkClient(registeredClient: RegisteredClient) {
        if (AuthorizationGrantType(CoreConstant.GRANT_TYPE_CUSTOM_CAPTCHA) !in registeredClient.authorizationGrantTypes) {
