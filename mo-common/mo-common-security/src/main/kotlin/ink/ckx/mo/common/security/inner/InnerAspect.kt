...
class InnerAspect(private val request: HttpServletRequest) {
    ...
    fun around(point: JoinPoint, inner: Inner?) {
        ...
        val innerAnnotation = inner ?: AnnotationUtils.findAnnotation(point.target.javaClass, Inner::class.java)
        val header = request.getHeader(CoreConstant.FROM)
        if (innerAnnotation == null || (innerAnnotation.value && CoreConstant.FROM_IN != header)) {
