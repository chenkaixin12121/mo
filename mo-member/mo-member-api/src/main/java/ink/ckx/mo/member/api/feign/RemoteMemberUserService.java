 ...
 * @since 2023/11/11
 */
@FeignClient(value = "mo-member", contextId = "member-user", fallback = RemoteMemberUserServiceFallback.class, configuration = {FeignDecoderConfig.class})
public interface RemoteMemberUserService {

    @GetMapping("/appApi/v1/users/{mobile}/authInfo")
    MemberUserInfoDTO getUserInfo(@PathVariable String mobile, @RequestHeader(CoreConstant.FROM) String from);
}